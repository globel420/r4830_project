...
## 37) Output Status UI Sync After Explicit Commands (2026-02-09T23:31:24Z)
- Addressed issue where output commands worked but Hub state label stayed `Disabled`.
- State-sync behavior updated for output status:
  - keep local commanded output state (`_outputEnabledLocal`) unless telemetry explicitly matches it
  - clear local output state on disconnect to avoid stale carryover across sessions
- This allows UI to reflect recent user command even when telemetry readback lags or is ambiguous.
- Added widget regression coverage:
  - verifies output status switches to `Enabled` after tapping enable even when telemetry still reports disabled.
- Files:
  - `/Users/globel/r4830_project/swift/lib/ui/screens/hub_screen.dart`
  - `/Users/globel/r4830_project/swift/test/widget_test.dart`
- Validation:
